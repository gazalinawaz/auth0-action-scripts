...
export class AuthService {
  ...
  private readonly REDIRECT_URI = 'https://portal.nyseg.com/medical/login';
  private readonly OPCO = 'nyseg'; // Change per portal: cng, scg, uinet, nyseg, rge
  ...
  login(): void {
    ...
    const authUrl = `https://${this.AUTH0_DOMAIN}/authorize` +
      ...
      `&scope=openid profile email` +
      `&state=${state}` +
      `&ext-opco=${this.OPCO}`; // ✅ Pass opco identifier
...
const REDIRECT_URI = 'https://portal.nyseg.com/medical/login';
const OPCO = 'nyseg'; // Change per portal
